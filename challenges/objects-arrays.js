...
/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
 const unisWithUni = graduates.filter(el => (el.university.indexOf('Uni') != -1));
...
*/
const displayNames = []; // zooAnimals.forEach((el) => `Name: ${el.animal_name}, Scientific: ${el.scientific_name}.`);
zooAnimals.forEach((el) => displayNames.push(`Name: ${el.animal_name}, Scientific: ${el.scientific_name}.`));
...
const lowCaseAnimalNames = zooAnimals.map(el => `${el.animal_name.toLowerCase()}`);
...
*/
const populationTotal = zooAnimals.reduce((total, el) => total + el.population, 0);
